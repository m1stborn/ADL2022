...
def main(args):
    ...
    splits = ["train", "eval"]
    raw_datasets = load_dataset("title_gen.py", name="Title Gen", cache_dir="./cache",
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in epoch_pbar:
            ...
            if step % args.gradient_accumulation_steps == 0 or step == len(train_dataloader) - 1:
                ...
                # Set Progress Bar
                epoch_pbar.set_description(f'Epoch[{epoch + 1}/{args.num_train_epochs}]')
...
if __name__ == "__main__":
    arg = parse_args()
    arg.ckpt_dir = Path(f"{arg.ckpt_dir}/{UID[:8]}")
    arg.ckpt_dir.mkdir(parents=True, exist_ok=True)
    main(arg)
